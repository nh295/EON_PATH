...
(deffunction sufficient-performance-Taurus-XL (?m ?type ?h ?i)
    ...
    (if (eq ?type SSO)  then
      ...
      ;; LEO equatorial
        ...
        (bind ?perf (- 1716 (* 0.62 ?h ))); LEO polar perf for Ariane5 in kg from user manual page 2-12
        (if (> ?perf (* ?m 1.1)) then ; 30% margin
            (return 1)
        else (return 0))
...
(defrule LV-SELECTION::compute-packaging-efficiency    
     "Compute packaging factor for launch vehicle/sat/orbit combination"
    
    ?f <- (MANIFEST::Mission (Name ?miss) (launch-vehicle ?lv&~nil) (satellite-mass# ?m&~nil) 
        (orbit-type ?typ&~nil) (orbit-altitude# ?h&~nil) (orbit-inclination ?i&~nil) 
        (lv-pack-efficiency# nil))
    =>
    ;(printout t "Pack factor of lv " ?lv " for sat with mass " ?m  crlf)
    (bind ?fac (/ ?m (get-max-perf-lv ?lv ?typ ?h)))
    ;(printout t "Pack factor of lv " ?lv " for sat with mass " ?m " is " ?fac crlf)
    (modify ?f (lv-pack-efficiency# ?fac))
    )
