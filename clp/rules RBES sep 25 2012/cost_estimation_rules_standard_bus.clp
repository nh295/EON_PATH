...
; use database of standard buses
...
(defrule COST-ESTIMATION::estimate-bus-non-recurring-cost
    ...
    ?miss <- (MANIFEST::Mission (bus-non-recurring-cost# nil)
        ...
        (structure-mass# ?strm &~nil) (propulsion-mass# ?prm&~nil) (comm-OBDH-mass# ?comm&~nil)
...
(defrule COST-ESTIMATION::estimate-bus-TFU-recurring-cost
    ...
    ?miss <- (MANIFEST::Mission (bus-recurring-cost# nil)
        (EPS-mass# ?epsm&~nil) (thermal-mass# ?thm&~nil) (comm-OBDH-mass# ?comm&~nil)
...
; spacecraft recurring and non-recurring s/c cost
(defrule COST-ESTIMATION::estimate-spacecraft-cost-standard
    "This rule estimates s/c non recurring cost adding up bus and payload n/r cost"
    (declare (salience 5))
    ?miss <- (MANIFEST::Mission (spacecraft-non-recurring-cost# nil) (spacecraft-recurring-cost# nil)  (bus-cost# ?bus&~nil)  (payload-cost# ?payl&~nil) (standard-bus ?sbus&~nil&~dedicated-class)
        )
    =>
    (bind ?spacecraftnr (+ (* ?bus 0.6) (* ?payl 0.6)))
    (bind ?spacecraft (+ (* ?bus 0.4) (* ?payl 0.4)))
    (bind ?sat (+ ?spacecraftnr ?spacecraft))
    (modify ?miss (spacecraft-non-recurring-cost# ?spacecraftnr)
         (spacecraft-recurring-cost# ?spacecraft) (satellite-cost# ?sat))
    )
...
(defrule COST-ESTIMATION::estimate-integration-and-testing-cost
    ...
    ?miss <- (MANIFEST::Mission (IAT-non-recurring-cost# nil) (IAT-recurring-cost# nil) (IAT-cost# nil) 
        (spacecraft-non-recurring-cost# ?scnr&~nil) (satellite-mass# ?m&~nil)
        )
    =>
...
(defrule COST-ESTIMATION::estimate-operations-cost
...
(defrule COST-ESTIMATION::estimate-total-mission-cost-with-overruns
    ...
    (bind ?over (matlabf compute_cost_overrun (get-instrument-list-trls ?ins)))
