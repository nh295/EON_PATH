...
(defrule MANIFEST::compute-MWR-spatial-resolution
    ...
    (bind ?sw (* 2 (* (/ ?h (matlabf cos (torad ?theta))) (matlabf tan (/ ?alfa 2)))))
...
(defrule MANIFEST::compute-RAD-spatial-resolution
    ...
    (bind ?sw (* 2 (* (/ ?h (matlabf cos (torad ?theta))) (matlabf tan (/ ?alfa 2)))))
    (modify ?RAD (Horizontal-Spatial-Resolution# (* ?nl ?range-res)) (Horizontal-Spatial-Resolution-Along-track# (/ ?range-res (matlabf sin (torad ?theta)))) 
