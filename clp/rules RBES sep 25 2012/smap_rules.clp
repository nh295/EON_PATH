...
(deftemplate ENUMERATION::SMAP-ARCHITECTURE (multislot payload) (slot num-sats)
    (slot orbit-altitude) (slot orbit-raan) (slot orbit-type) (slot orbit-inc) (slot num-planes)
    (multislot doesnt-fly) (slot num-sats-per-plane)
    (slot num-instruments) (multislot sat-assignments))
...
(defrule ENUMERATION::duplicate-arch-add-instrument
    ?a <- (SMAP-ARCHITECTURE (payload $?payl) (num-instruments ?N&:(< ?N (length$ (get-instruments))))
...
(defrule ENUMERATION::duplicate-arch-add-constellation-design
    ?a <- (SMAP-ARCHITECTURE (num-planes nil) (num-sats-per-plane nil) (num-instruments ?N&~0))
...
(defrule ENUMERATION::duplicate-arch-add-orbit-altitude
    ?a <- (SMAP-ARCHITECTURE (orbit-altitude nil) (num-instruments ?N&~0))
...
(defrule ENUMERATION::duplicate-arch-add-orbit-RAAN
    ?a <- (SMAP-ARCHITECTURE (orbit-raan nil) (orbit-inc SSO) (num-instruments ?N&~0))
...
(defrule ENUMERATION::duplicate-arch-add-orbit-RAAN-NA
    ?a <- (SMAP-ARCHITECTURE (orbit-raan nil) (orbit-inc ?inc&~SSO&~nil) (num-instruments ?N&~0))
...
(defrule ENUMERATION::duplicate-arch-add-orbit-inclination-and-type
    ?a <- (SMAP-ARCHITECTURE (orbit-inc nil) (num-instruments ?N&~0))
...
(defrule ENUMERATION::duplicate-arch-add-nsats
    ?a <- (SMAP-ARCHITECTURE (num-instruments ?N&~0)  (sat-assignments $?ass))      
    (test (< (length$ $?ass) ?N))
...
(defrule ENUMERATION::remove-incomplete-arch-missing-raan
    (declare (salience -10))
    ?a <- (SMAP-ARCHITECTURE (orbit-raan nil))
    =>
    (retract ?a)
    )
...
(defrule ENUMERATION::remove-incomplete-arch-missing-inc
    (declare (salience -10))
    ?a <- (SMAP-ARCHITECTURE (orbit-inc nil))
    =>
    (retract ?a)
    )
...
(defrule ENUMERATION::remove-incomplete-arch-missing-type
    (declare (salience -10))
    ?a <- (SMAP-ARCHITECTURE (orbit-type nil))
    =>
    (retract ?a)
    )
...
(defrule ENUMERATION::remove-incomplete-arch-missing-numsats
    (declare (salience -10))
    ?a <- (SMAP-ARCHITECTURE (num-sats-per-plane nil))
    =>
    (retract ?a)
    )
...
(defrule ENUMERATION::remove-incomplete-arch-missing-numplanes
    (declare (salience -10))
    ?a <- (SMAP-ARCHITECTURE (num-planes nil))
    =>
    (retract ?a)
    )
...
(defrule ENUMERATION::remove-incomplete-arch-missing-altitude
    (declare (salience -10))
    ?a <- (SMAP-ARCHITECTURE (orbit-altitude nil))
    =>
    (retract ?a)
    )
...
(defrule ENUMERATION::remove-incomplete-arch-missing-nsats
    (declare (salience -10))
    ?a <- (SMAP-ARCHITECTURE (num-sats nil))
    =>
    (retract ?a)
    )
...
(defrule MANIFEST::assert-missions
    (ENUMERATION::SMAP-ARCHITECTURE (payload $?p) (num-sats ?ns) (orbit-type ?typ)
    ...
    (for (bind ?i 2) (<= ?i (length$ ?sats)) (++ ?i)
        ...
        (if (eq "sat" ?el) then 
            ...
            (assert (MANIFEST::Mission (Name (str-cat "SMAP-" ?nsat))  (mission-architecture ?march) 
                    (num-of-planes# ?np) (num-of-sats-per-plane# ?nsp) (orbit-type ?typ)
    ...
    (assert (MANIFEST::Mission (Name (str-cat "SMAP-" ?nsat)) (lifetime 5) (mission-architecture ?march) 
            ...
            (instruments ?payl) (orbit-altitude# ?h)
