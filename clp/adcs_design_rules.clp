...
(defrule MASS-BUDGET::set-ADCS-type
    ?sat <- (MANIFEST::Mission (ADCS-requirement ?adcs&~nil) (ADCS-type nil))
    =>
...
(defrule MASS-BUDGET::estimate-drag-coefficient
    ?sat <- (MANIFEST::Mission (drag-coefficient nil) (satellite-dimensions $?dim&:(> (length$ $?dim) 0)))
    =>
...
(defrule MASS-BUDGET::estimate-residual-dipole
    ...
    ?sat <- (MANIFEST::Mission (residual-dipole nil))
    =>
