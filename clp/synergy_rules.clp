...
(defrule SYNERGIES::spatial-disaggregation-hyperspectral "A hyperspectral coarse spatial resolution measurement can be combined with a multispectral high spatial resolution measurement to produce a high spatial resolution hyperspectral measurement with lower accuracy"
    ?m1 <- (REQUIREMENTS::Measurement (Parameter ?p&~nil) (Temporal-resolution ?tr1&~nil) (Spectral-sampling Multispectral-10-100-channels) (Horizontal-Spatial-Resolution ?hsr1&~nil) (Accuracy ?a1&~nil) (Id ?id1) (taken-by ?ins1) (synergy-level# ?s1&:(< ?s1 1)))
    ?m2 <- (REQUIREMENTS::Measurement (Parameter ?p&~nil) (Temporal-resolution ?tr2&~nil) (Spectral-sampling Hyperspectral-100-channels-or-more) (Horizontal-Spatial-Resolution ?hsr2&~nil) (Accuracy ?a2&~nil) (Id ?id2) (taken-by ?ins2) (synergy-level# ?s2&:(< ?s2 1)))
    (SYNERGIES::cross-registered (measurements $?m))
    (test (member$ ?id1 $?m))
    (test (member$ ?id2 $?m))
    (not (REASONING::stop-improving (Measurement ?p)))
...
(defrule SYNERGIES::spatial-averaging "any image can be averaged out in space to provide a new, better accuracy, coarser resolution image"
    ...
    (duplicate ?m (Id (str-cat ?id1 "-space-averaged")) (Horizontal-Spatial-Resolution (eval (Worsen Horizontal-Spatial-Resolution ?hsr1))) (Accuracy (eval (Improve Accuracy ?a2))) (taken-by (str-cat ?ins1 "-space-averaged")) (synergy-level# (+ ?s1 1)))
)
...
(defrule SYNERGIES::time-averaging "any image can be averaged out in time to provide a new, better accuracy, sparser temporal resolution image"
    ...
    (not (REASONING::stop-improving (Measurement ?p)))
	=>
    (duplicate ?m (Id (str-cat ?id1 "-time-averaged")) (Temporal-resolution (eval (Worsen Temporal-resolution ?tr1))) (Accuracy (eval (Improve Accuracy ?a2))) (taken-by (str-cat ?ins1 "-time-averaged")) (synergy-level# (+ ?s1 1)))
)
...
(defrule SYNERGIES::seafloor-topography 
    ...
    =>
    (duplicate ?slh (Id (str-cat ?id1 "-syn-" ?id2)) (Parameter "3.2.2 seafloor topography")))
...
(defrule SYNERGIES::river-plumes-from-ocean-color "River plumes and sediment fluxes can be measured with an ocean color measurement of sufficient horizontal spatial resolution"
    ...
    (REQUIREMENTS::Measurement (Parameter "3.1.2 Extended ocean color - UV (enhanced DOC, CDOM)") (Horizontal-Spatial-Resolution ?hsr & :(neq ?hsr nil)) (Id ?id2))
    (SYNERGIES::cross-registered (measurements $?m))
    (test (member$ ?id1 $?m))
    (test (member$ ?id2 $?m))
    =>
    (duplicate ?oc (Id (str-cat ?id1 "-syn")) (Parameter "3.2.5 river plumes/sediment fluxes")))
...
(defrule SYNERGIES::hydrocarbon-reservoir-monitoring-from-surface-deformation "Hydrocarbon reservoirs can be monitored by measuring surface deformation and surface composition"
    ...
    (test (SameOrBetter Horizontal-Spatial-Resolution ?hsr High-10-100m))
    (REQUIREMENTS::Measurement (Parameter "2.6.5 surface composition") (Horizontal-Spatial-Resolution ?hsr & :(neq ?hsr nil)) (Id ?id2))
    (SYNERGIES::cross-registered (measurements $?m))
    (test (member$ ?id1 $?m))
    (test (member$ ?id2 $?m))
    =>
    (duplicate ?sd (Id (str-cat ?id1 "-syn-" ?id2)) (Parameter "2.6.4 hydrocarbon reservoir monitoring")))
...
(defrule SYNERGIES::flood-monitoring-from-hires-topography "Hi res topography with 5m horizontal spatial resolution and 10cm accuracy allows flood monitoring"
    ...
    =>
    (duplicate ?topo (Id (str-cat ?id1 "-syn")) (Parameter "2.6.4 hydrocarbon reservoir monitoring")))
...
(defrule SYNERGIES::groundwater-storage-from-gravity "Groundwater storage can be inferred from precise gravity measurements"
    ...
    =>
    (duplicate ?grav (Id (str-cat ?id1 "-syn")) (Parameter "2.7.3 groundwater storage")))
...
(defrule SYNERGIES::glacier-mass-balance-from-gravity "Glacier mass balance measurements can be inferred from precise gravity measurements using ice topography measurements"
    	...
    	(test (member$ ?id2 $?m))
    =>
    (duplicate ?topo (Id (str-cat ?id1 "-syn" ?id2)) (taken-by (str-cat ?ins1 "-syn" ?ins2)) (Parameter "4.1.3 glacier mass balance")))
...
(defrule SYNERGIES::ocean-mass-distribution-from-gravity "Ocean mass distribution can be inferred from precise gravity measurements"
        ?grav <- (REQUIREMENTS::Measurement (Parameter "5.1.1 Geoid and gravity field variations") (Horizontal-Spatial-Resolution ?hsr & :(neq ?hsr nil)) (Id ?id1))
    =>
    (duplicate ?grav (Id (str-cat ?id1 "-syn")) (Parameter "3.2.6 Ocean mass distribution")))
...
(defrule SYNERGIES::visible-atmospheric-plume-from-aerosols "Visible atmospheric plumes can be measured from high temporal and spatial resolution multispectral aerosol measurements"
    ...
    (test (SameOrBetter Temporal-resolution ?tr High-12h-24h))
    =>
    (duplicate ?ae (Id (str-cat ?id1 "-syn")) (Parameter "1.8.16 Visible atmospheric plumes")))
...
(defrule SYNERGIES::black-carbon-from-aerosols "Black carbon can be measured from polarimetric aerosol measurements"
    ...
    =>
    (duplicate ?ae (Id (str-cat ?id1 "-syn")) (Parameter "1.8.13 Black carbon and other polluting aerosols")))
...
(defrule SYNERGIES::surface-composition-in-all-spectrum
    ...
    =>
    (duplicate ?TIR (Spectral-region opt-VNIR+SWIR+TIR) (Temporal-resolution (fuzzy-min Temporal-resolution ?tr1 ?tr2)) (Spectral-sampling (fuzzy-max Spectral-sampling ?ss1 ?ss2)) (taken-by (str-cat ?ins1 "-syn-" ?ins2 )) (Id (str-cat ?id1 "-syn-" ?id2)) )
    )
...
(defrule SYNERGIES::add-all-weather-capability
    ...
    (REQUIREMENTS::Measurement (Parameter ?p) (All-weather yes) (Id ?id2) (taken-by ?ins2) (synergy-level# ?s2&:(< ?s2 1)))
    ...
    (duplicate ?no (All-weather yes) (Id (str-cat ?id1 "-syn-" ?id2))
        (taken-by (str-cat ?ins1 "-syn-" ?ins2 )) (synergy-level# (+ (max ?s1 ?s2) 1)))
    )
...
(defrule SYNERGIES::add-cloud-mask
    ...
    ?no <- (REQUIREMENTS::Measurement (Parameter ?p) (cloud-cleared no) (Id ?id1) (taken-by ?ins1) (synergy-level# ?s1&:(< ?s1 2)))
    (REQUIREMENTS::Measurement (Parameter "1.5.4 cloud mask") (Id ?id2) (taken-by ?ins2) (synergy-level# ?s2&:(< ?s2 1)))
    (SYNERGIES::cross-registered (measurements $?m))
    (test (member$ ?id1 $?m))
    (test (member$ ?id2 $?m))
    
    =>
    (modify ?no (cloud-cleared yes) (Id (str-cat ?id1 "-mask-" ?id2)) (taken-by (str-cat ?ins1 "-mask-" ?ins2 )) (synergy-level# (+ 1 (max ?s1 ?s2))))
    (assert (SYNERGIES::cross-registered (measurements (insert$ $?m (+ 1 (length$ $?m)) (str-cat ?id1 "-mask-" ?id2 )))))
    )
...
(defrule SYNERGIES::add-sensitivity-in-cirrus-from-mmw-measurement
    ...
    (REQUIREMENTS::Measurement (Parameter ?p) (sensitivity-in-cirrus High) (Id ?id2) (taken-by ?ins2) (synergy-level# ?s2&:(< ?s2 1)))
    (not (REASONING::stop-improving (Measurement ?p)))
    ;(test (eq (str-index syn ?ins1) FALSE))
    ;(test (eq (str-index syn ?ins2) FALSE))
    ;(SYNERGIES::cross-registered (measurements $?m))
    ...
    (duplicate ?no (sensitivity-in-cirrus High) (Id (str-cat ?id1 "-syn-" ?id2))
        (taken-by (str-cat ?ins1 "-syn-" ?ins2 )) (synergy-level# (+ 1 (max ?s1 ?s2))))
    )
...
(defrule SYNERGIES::add-sensitivity-in-convective-clouds
    ...
    (REQUIREMENTS::Measurement (Parameter ?p) (sensitivity-in-convective-clouds High) (Id ?id2) (taken-by ?ins2) (synergy-level# ?s2&:(< ?s2 1)))
    (not (REASONING::stop-improving (Measurement ?p)))
    ;(test (eq (str-index syn ?ins1) FALSE))
    ;(test (eq (str-index syn ?ins2) FALSE))
    ;(SYNERGIES::cross-registered (measurements $?m))
...
(defrule SYNERGIES::column-vs-profile-chemistry-measurements
    ...
    (duplicate ?col (Accuracy (eval (Improve Accuracy ?acc1))) (Id (str-cat ?id1 "-syn-" ?id2))
        (taken-by (str-cat ?ins1 "-syn-" ?ins2 )) (Vertical-Spatial-Resolution ?vsr) (synergy-level# (+ 1 (max ?s1 ?s2))))
...
(defrule SYNERGIES::sensitivity-over-oceans
    ...
    (duplicate ?no (Id (str-cat ?id1 "-syn-" ?id2)) (sensitivity-over-oceans High)
        (taken-by (str-cat ?ins1 "-syn-" ?ins2 )) (synergy-level# (+ 1 (max ?s2 ?s1))))
...
Occultation Events for Orbit Selection for Global/Regional Observation, RAST 2007"
    ...
    ?m1 <- (REQUIREMENTS::Measurement (Parameter "1.3.3 GPS radio occultation") (num-soundings-per-day# ?ns1&~nil) (taken-by ?tk1))
    ?m2 <- (REQUIREMENTS::Measurement (Parameter "1.3.3 GPS radio occultation") (num-soundings-per-day# ?ns2&~nil) (taken-by ?tk2))
    (test (neq ?m1 ?m2))
    => 
    (retract ?m1)
    (modify ?m2 (num-soundings-per-day# (+ ?ns1 ?ns2)) (taken-by (str-cat ?tk1 ?tk2)))
...
(defrule altimetry-err-budget-POD
    "If an instrument providing precise orbit determinations is sharing a platform with an altimeter, 
    then the POD contribution in the altimetry error budget is decreased to 2 cm if the orbit is high 
    enough to have low drag, and 4 cm if it is a lower orbit, i.e. 800km or less"
    (declare (no-loop TRUE))
    ?meas <- (REQUIREMENTS::Measurement (Parameter "3.2.1 Sea level height") 
        (orbit-altitude# ?h) (rms-system-POD# 10.0) (taken-by ?ra))
    (REQUIREMENTS::Measurement (Parameter "A9.Precise Orbit Determination") (taken-by ?gps) (Id ?id2))
    ...
    (modify ?meas (rms-system-POD# ?new-rms-POD) (taken-by (str-cat ?ra "-POD-" ?gps)))
    )

(defrule altimetry-err-budget-tropo-wet-correction
    "If an atmospheric humidity measurement is cross-correlated with an altimetry measurement, 
    the tropospheric correction contribution in the altimetry error budget is decreased by a 
    factor that is directly proportional to the accuracy of the humidity measurement"
    (declare (no-loop TRUE))
    ?meas <- (REQUIREMENTS::Measurement (Parameter "3.2.1 Sea level height") (taken-by ?ra) (rms-system-tropoH2O# ?rms-tropo&:(>= ?rms-tropo 1)) (Id ?id1))
    (REQUIREMENTS::Measurement (Parameter "1.8.1 H2O") (Id ?id2) (taken-by ?mwr) (rms-system-instrument# ?rms-mwr &~nil)); 
    (SYNERGIES::cross-registered (measurements $?m))
    (test (member$ ?id1 $?m))
    (test (member$ ?id2 $?m))
    =>
    (bind ?new-rms-tropo (* ?rms-mwr 0.6)); 0.6cm per K of rms error in the MWR (christensen et al 93)
    (modify ?meas (rms-system-tropoH2O# ?new-rms-tropo) (taken-by (str-cat ?ra "-wet-" ?mwr)))
    )

(defrule altimetry-err-budget-ionospheric-correction
    "If a measurement of inospheric total electron content is cross-correlated with an altimetry measurement, 
    the ionospheric correction contribution in the altimetry error budget is decreased by a 
    factor that is directly proportional to the accuracy of the inospheric instrument"
    (declare (no-loop TRUE))
    ?meas <- (REQUIREMENTS::Measurement (Parameter "3.2.1 Sea level height") (taken-by ?ra) (rms-system-ionosphere# ?rms-iono&:(>= ?rms-iono 1)) (Id ?id1))
   (REQUIREMENTS::Measurement (Parameter "A8.Total electron content in ionosphere") (Accuracy ?acc) (Id ?id2) (taken-by ?io))
    (SYNERGIES::cross-registered (measurements $?m))
    (test (member$ ?id1 $?m))
    (test (member$ ?id2 $?m))
    =>
    (if (eq ?acc High) then (bind ?new-rms-iono (/ ?rms-iono 5)) else (bind ?new-rms-iono (/ ?rms-iono 3))); assume for now that the effect of instrument is to divide contribution to error by 3
    (modify ?meas (rms-system-ionosphere# ?new-rms-iono) (taken-by (str-cat ?ra "-iono-" ?io)))
    )
...
(defrule SYNERGIES::altimetry-variable-error
   "If several independent measurements are gathered for example in a constellation, 
    the variable ensemble measurement error decreases as 1/sqtr(2+N), assuming 2 altimeters
    are already being flown (conversation w/ Dr Sarah Gille, UCSD)"
    (declare (no-loop TRUE))
    ?meas <- (REQUIREMENTS::Measurement (Parameter "3.2.1 Sea level height")
         (rms-variable-measurement# ?rms-var&4.0) (num-of-planes# ?np&~nil) (taken-by ?ra)
        (num-of-sats-per-plane# ?ns&~nil))
    
    =>
    (bind ?new-rms-var (/ ?rms-var (sqrt (+ 2 (* ?np ?ns))))); assume that there are 2 other altimeters
...
(defrule SYNERGIES::altimetry-variable-error-when-nil
   "If several independent measurements are gathered for example in a constellation, 
    ...
    =>
    (bind ?new-rms-var (/ ?rms-var (sqrt 3))); assume that there are 2 other altimeters
    ;(printout t rms-variable-measurement# " " ?new-rms-var crlf)
    (modify ?meas (rms-variable-measurement# ?new-rms-var))
    )


(defrule altimetry-err-budget-tropo-dry-correction
    "If an atmospheric pressure measurement is cross-correlated with an altimetry measurement, 
    the dry tropospheric correction contribution in the altimetry error budget is decreased by a 
    factor that is directly proportional to the accuracy of the pressure measurement"
    (declare (no-loop TRUE))
    ?meas <- (REQUIREMENTS::Measurement (Parameter "3.2.1 Sea level height") (taken-by ?ra) (rms-system-tropo-dry# ?rms-tropo&:(>= ?rms-tropo 0.5)) (Id ?id1))
    (REQUIREMENTS::Measurement (Parameter "1.3.4 Atmospheric pressure") (Id ?id2) (taken-by ?dry) (rms-system-instrument# ?rms-mwr &~nil))
    (SYNERGIES::cross-registered (measurements $?m))
    (test (member$ ?id1 $?m))
    (test (member$ ?id2 $?m))
    =>
    (bind ?new-rms-tropo (* ?rms-mwr 0.2)); 0.2cm per K of rms error in the MWR
    (modify ?meas (rms-system-tropo-dry# ?new-rms-tropo) (taken-by (str-cat ?ra "-dry-" ?dry)))
    )
...
(defrule SYNERGIES::altimetry-err-total
    "If an instrument providing precise orbit determinations is sharing a platform with an altimeter, 
    then the POD contribution in the altimetry error budget is decreased from 10-15cm to 2-5 cm"
    (declare (salience -30) (no-loop TRUE) )
    ?meas <- (REQUIREMENTS::Measurement (Parameter "3.2.1 Sea level height")
         (rms-system-POD# ?rms-POD)  (rms-system-tropoH2O# ?rms-tropo) (rms-system-ionosphere# ?rms-iono)
        (rms-system-instrument# ?rms-ins) (rms-variable-measurement# ?rms-var) 
        (rms-system-tides# ?rms-tides) (rms-system-tropo-dry# ?rms-tropo-dry) (rms-total# 100.0))

    =>
    (bind ?rms-total (sqrt (+ (** ?rms-POD 2) (** ?rms-iono 2) (** ?rms-tropo 2)  (** ?rms-ins 2)
       (** ?rms-tides 2) (** ?rms-tropo-dry 2)  (** ?rms-var 2))))
    (modify ?meas (rms-total# ?rms-total))
    )


;(deftemplate SYNERGIES::need-error-budget (slot Parameter))

(defquery SYNERGIES::get-error-budget
    "Get error budget controbutions from measurement"
    (declare (variables ?p))
    (REQUIREMENTS::Measurement (Parameter ?p)  (rms-system-POD# ?rms-POD) 
         (rms-system-tropoH2O# ?rms-tropo) (rms-system-ionosphere# ?rms-iono)
        (rms-system-instrument# ?rms-ins) (rms-variable-measurement# ?rms-var) 
        (rms-total# ?rms-total) (rms-system-tides# ?rms-tide)
         (rms-system-tropo-dry# ?rms-dry))
    )
...
(defrule SYNERGIES::clouds-and-radiation2
    ...
    (CAPABILITIES::Manifested-instrument (Intent "Earth radiation budget radiometers") (Name ?n1&~ACRIM) )
    (CAPABILITIES::Manifested-instrument (Intent "Earth radiation budget radiometers") (Name ?n2&~?n1&~ACRIM) )
    (REQUIREMENTS::Measurement (Parameter "1.9.3 Spectrally resolved SW radiance -0.3-2um-") (Id ?id1) (taken-by ?ins1)) 
...
(defrule SYNERGIES::clouds-and-radiation3
    ...
    (CAPABILITIES::Manifested-instrument (Intent "Earth radiation budget radiometers") (Name ?n1&~ACRIM))
    (CAPABILITIES::Manifested-instrument (Intent "Earth radiation budget radiometers") (Name ?n2&~?n1&~ACRIM))
    (CAPABILITIES::Manifested-instrument (Intent "Earth radiation budget radiometers") (Name ?n3&~?n2&~?n1&~ACRIM))
    (REQUIREMENTS::Measurement (Parameter "1.9.3 Spectrally resolved SW radiance -0.3-2um-") (Id ?id1) (taken-by ?ins1)) 
    ?clouds <- (REQUIREMENTS::Measurement (Parameter "1.5.3 Cloud amount/distribution -horizontal and vertical-") (Id ?id2) (taken-by ?ins2))
    (SYNERGIES::cross-registered (measurements $?m))
    (test (member$ ?id1 $?m))
    ...
    (duplicate ?clouds (Parameter "A4.Clouds and radiation") (num-of-indep-samples# 3)
        (Id (str-cat ?id1 "-syn-" ?id2))  (taken-by (str-cat ?ins1 "-syn-" ?ins2 "-syn" ?n1 "-syn" ?n2 "-syn" ?n3)))
...
(defrule SYNERGIES::CERES-sampling-multiple-angles-SW 
    ...
    (declare (no-loop TRUE))
    (CAPABILITIES::Manifested-instrument (Intent "Earth radiation budget radiometers") (scanning cross-track) (Name ?n1))
    (CAPABILITIES::Manifested-instrument (Intent "Earth radiation budget radiometers") (scanning biaxial) (Name ?n2&:(neq ?n1 ?n2)))
    ?m <- (REQUIREMENTS::Measurement (Parameter "1.9.3 Spectrally resolved SW radiance -0.3-2um-") (rms-variable-angular-sampling# ?ang))
    (test (eq (numberp ?ang) FALSE));; essentially nil
    =>
    (modify ?m (rms-variable-angular-sampling# 1.2)) ;; 1sigma, in W/m2 averaged over 30 days, Wielicki et al 1995
    )
...
(defrule SYNERGIES::CERES-sampling-multiple-angles-LW
    ...
    (CAPABILITIES::Manifested-instrument (Intent "Earth radiation budget radiometers") (scanning cross-track) (Name ?n1))
    (CAPABILITIES::Manifested-instrument (Intent "Earth radiation budget radiometers") (scanning biaxial) (Name ?n2&:(neq ?n1 ?n2)))
    ?m <- (REQUIREMENTS::Measurement (Parameter "1.9.2 Spectrally resolved IR radiance -200-2000cm-1-") (rms-variable-angular-sampling# ?ang))
    (test (eq (numberp ?ang) FALSE));; essentially nil
    =>
    (modify ?m (rms-variable-angular-sampling# 1.2)) ;; 1sigma, in W/m2 averaged over 30 days, Wielicki et al 1995
    )
...
(defrule SYNERGIES::CERES-sampling-single-angle-SW
    ...
    (test (eq (numberp ?ang) FALSE));; essentially nil
    =>
    (modify ?m (rms-variable-angular-sampling# 4.8)) ;; 1sigma, in W/m2 averaged over 30 days, Wielicki et al 1995
    )
...
(defrule SYNERGIES::CERES-sampling-time-SW2
    ...
    (REQUIREMENTS::Measurement (Parameter "A4.Clouds and radiation") (num-of-indep-samples# ?n&2))
    ?m <- (REQUIREMENTS::Measurement (Parameter "1.9.3 Spectrally resolved SW radiance -0.3-2um-") (rms-variable-time-sampling# ?tim))
    ...
    (modify ?m (rms-variable-time-sampling# ?x)) ;; 1sigma, in W/m2 averaged over 30 days, Wielicki et al 1995
    )
...
(defrule SYNERGIES::CERES-sampling-time-LW2
    ...
    (declare (no-loop TRUE) (salience -3))
    (REQUIREMENTS::Measurement (Parameter "A4.Clouds and radiation") (num-of-indep-samples# ?n&2))
    ?m <- (REQUIREMENTS::Measurement (Parameter "1.9.2 Spectrally resolved IR radiance -200-2000cm-1-") (rms-variable-time-sampling# ?tim))
    (test (eq (numberp ?tim) FALSE));; essentially nil
    =>
    (if (eq ?n 1) then (bind ?x 10.45) elif (eq ?n 2) then (bind ?x 2.3) elif (eq ?n 3) then (bind ?x 0.47) else (bind ?x 15.0))
    (modify ?m (rms-variable-time-sampling# ?x)) ;; 1sigma, in W/m2 averaged over 30 days, Wielicki et al 1995
...
(defrule SYNERGIES::CERES-sampling-time-SW3
    ...
    sample all possible angles"
    (declare (no-loop TRUE) (salience -3))
    (REQUIREMENTS::Measurement (Parameter "A4.Clouds and radiation") (num-of-indep-samples# ?n&3))
    ?m <- (REQUIREMENTS::Measurement (Parameter "1.9.3 Spectrally resolved SW radiance -0.3-2um-") (rms-variable-time-sampling# ?tim))
    (test (eq (numberp ?tim) FALSE));; essentially nil
    =>
    (if (eq ?n 1) then (bind ?x 10.45) elif (eq ?n 2) then (bind ?x 2.3) elif (eq ?n 3) then (bind ?x 0.47) else (bind ?x 15.0))
...
(defrule SYNERGIES::radiation-budget-err-total-SW
    ...
    ?meas <- (REQUIREMENTS::Measurement (Parameter "1.9.3 Spectrally resolved SW radiance -0.3-2um-") (rms-variable-angular-sampling# ?ang)
         (rms-variable-time-sampling# ?tim) (rms-system-instrument# ?ins) (rms-total# 100.0))
...
(defrule SYNERGIES::sea-surface-winds-combining-SAR-and-scatterometer
    ...
    ?m2 <- (REQUIREMENTS::Measurement (Parameter "3.4.1 Ocean surface wind speed") (taken-by ?scat&:(neq ?scat ?sar)) (rms-variable-measurement# ?err2&~nil))
    (CAPABILITIES::Manifested-instrument (Intent "Imaging MW radars -SAR-") (Name ?sar))
...
(defrule SYNERGIES::sea-surface-winds-with-high-winds
    ...
    ?m <- (REQUIREMENTS::Measurement (Parameter "3.4.1 Ocean surface wind speed") (taken-by ?ins) (sensitivity-in-high-winds nil))
    (CAPABILITIES::Manifested-instrument (Intent "Radar scatterometer") (Name ?ins) (Spectral-region ?sp))
    =>
...
(defrule SYNERGIES::add-multi-angular-capability
    ...
    (REQUIREMENTS::Measurement (Parameter ?p) (taken-by ?ins2) (Id ?id2) (ThreeD ?td2 &~ No-3D &~ N-A &~ nil) (synergy-level# ?s2&:(< ?s2 1)))
    ...
    =>
     (duplicate ?no (ThreeD ?td2) (Id (str-cat ?id1 "-multiang-" ?id2))  (taken-by (str-cat ?ins1 "-multiang-" ?ins2 )) (synergy-level# (+ 1 (max ?s1 ?s2))))
    )
...
(defrule SYNERGIES::HSR-TR-ThreeD-combination-scheme
    ...
    ;(printout t HSR-TR-ThreeD-combination-scheme " " (str-cat ?ins1 "-disaggr-" ?ins2 "-multiang-" ?ins3) crlf)
     (duplicate ?MODIS (ThreeD Full-3D) (Horizontal-Spatial-Resolution High-10-100m)  (Id (str-cat ?id1 "-disagg-" ?id2 "-multiang-" ?id3))  (taken-by (str-cat ?ins1 "-disaggr-" ?ins2 "-multiang-" ?ins3)))
    )
...
(defrule SYNERGIES::temperature-sounding-suite
    ...
    ;(printout t temperature-sounding-suite crlf)
     (duplicate ?AIRS-MODIS (All-weather yes) (sensitivity-over-oceans High) (Id (str-cat ?id1 "-sound-" ?id2 )) (taken-by (str-cat ?ins1 "-sound-" ?ins2)) (synergy-level# (+ 1 (max ?s1 ?s2))))
    )
...
(defrule SYNERGIES::GRAVITY-measurements2 
   ...
   ?this <- (REQUIREMENTS::Measurement (Parameter "5.1.1 Geoid and gravity field variations") (Id ?id1)
         (taken-by GRAVITY) (orbit-altitude# ?h) (orbit-anomaly# ?ano) (orbit-RAAN ?raan) (flies-in ?miss) ) 
    (REQUIREMENTS::Measurement (Parameter "A9.Precise Orbit Determination") (taken-by CTECS) (flies-in ?miss) (Id ?id2) (orbit-altitude# ?h2) (orbit-anomaly# ?ano2) (orbit-RAAN ?raan2))
    (SYNERGIES::cross-registered (measurements $?m))
    (test (member$ ?id1 $?m))
    (test (member$ ?id2 $?m))
    
   => 
   (modify ?this (Accuracy High)) 
...
(defrule SYNERGIES::fire-monitoring
    ...
    (REQUIREMENTS::Measurement (Id ?id2) (taken-by ?ins2) (Parameter "1.8.3 CO2") (Spectral-sampling Multispectral-10-100-channels))
    ;(SYNERGIES::cross-registered (measurements $?m))
    ;(test (member$ ?id1 $?m))
    ;(test (member$ ?id2 $?m))
    =>
    ...
    (duplicate ?this (Parameter "A2.Fire Monitoring") (Id (str-cat ?id1 "-syn-" ?id2 )) (Accuracy Medium)
         (taken-by (str-cat ?ins1 "-syn-" ?ins2)) (Spectral-sampling Multispectral-10-100-channels))
    )
...
(defrule SYNERGIES::CO2-temperature-error
    ...
    ?CO2 <- (REQUIREMENTS::Measurement (Id ?id1) (taken-by ?ins1) (Parameter "1.8.3 CO2") (rms-system-tropoH2O# High))
    ...
    (modify ?CO2 (rms-system-tropoH2O# Low) (taken-by (str-cat ?ins1 "-syn-" ?ins2)) (Id (str-cat ?id1 "-syn-" ?id2 )))
    )

(defrule SYNERGIES::CO2-pressure-error
    "If a laser CO2 measurement error is accompanied by a passive O2 measurement 
    then the accuracy of the CO2 retrieval improves"
    ?CO2 <- (REQUIREMENTS::Measurement (Id ?id1) (taken-by ?ins1) (Parameter "1.8.3 CO2") (rms-system-tropo-dry# High))
    ...
    (modify ?CO2 (rms-system-tropo-dry# Low) (taken-by (str-cat ?ins1 "-syn-" ?ins2)) (Id (str-cat ?id1 "-syn-" ?id2 )))
    )
...
(defrule SYNERGIES::ocean-wind-vector-sensitivity-high-winds
    ...
    (duplicate ?Ku (sensitivity-in-high-winds High) (taken-by (str-cat ?ins1 "-syn-" ?ins2)) (Id (str-cat ?id1 "-syn-" ?id2 )))
    )
...
(defrule SYNERGIES::ocean-wind-vector-complete
    ...
    (duplicate ?Ku (sensitivity-in-high-winds High) (sensitivity-in-rain High) (rms-system-tropoH2O# Low) (taken-by (str-cat ?ins1 "-syn-" ?ins2 "-syn-" ?ins3)) (Id (str-cat ?id1 "-syn-" ?id2 "-syn-" ?id3)))
    )
...
(defrule SYNERGIES::ocean-wind-vector-sensitivity-in-rain
    ...
    (duplicate ?Ku (sensitivity-in-rain High) (taken-by (str-cat ?ins1 "-syn-" ?ins2)) (Id (str-cat ?id1 "-syn-" ?id2 )))
    )
...
(defrule SYNERGIES::ocean-wind-vector-atmospheric-correction
    ...
    (duplicate ?Ku (rms-system-tropoH2O# Low) (taken-by (str-cat ?ins1 "-syn-" ?ins2)) (Id (str-cat ?id1 "-syn-" ?id2 )))
    )
...
(defrule SYNERGIES::cross-instrument-calibration-heritage-SCLP
    ...
    ?SAR <- (REQUIREMENTS::Measurement (Parameter "4.2.1 snow-water equivalence") (Illumination Active) (On-board-calibration ?obc&~High) (taken-by ?ins1) (Id ?id1))
    (REQUIREMENTS::Measurement (Parameter "4.2.1 snow-water equivalence") (Illumination Passive) (taken-by ?ins2) (Id ?id2))
    (SYNERGIES::cross-registered (measurements $?m))
    (test (member$ ?id1 $?m))
    (test (member$ ?id2 $?m))
    =>
...
(defrule SYNERGIES::several-lidars-for-aerosols
    ...
    (REQUIREMENTS::Measurement (Parameter "1.1.4 aerosol extinction profiles/vertical concentration") (orbit-altitude# ?h2&~nil) (orbit-type ?typ2) 
        (orbit-RAAN ?raan2) (orbit-inclination ?inc2) (Temporal-resolution ?tr2)  (taken-by ?ins2&~?ins1) (Id ?id2&~?id1))
    (REQUIREMENTS::Measurement (Parameter "1.1.4 aerosol extinction profiles/vertical concentration") (orbit-altitude# ?h3&~nil) (orbit-type ?typ3) 
    ...
    (duplicate ?m1 (Temporal-resolution Medium-1day-3days) (taken-by (str-cat ?ins1 "-syn-" ?ins2 "-syn-" ?ins3)) (Id (str-cat ?id1 "-syn-" ?id2 "-syn-" ?id3)))
    )
...
(defrule SYNERGIES::NOx-strategy
    ...
    (REQUIREMENTS::Measurement (Parameter "1.5.4 cloud mask") (Id ?id3) (taken-by ?ins3) (synergy-level# ?s3&:(< ?s3 1)))
...
(defrule SYNERGIES::water-vapor-transport-strategy
    ...
    (duplicate ?PATH (Region-of-interest Global) (Vertical-Spatial-Resolution High-200m-orless) (Horizontal-Spatial-Resolution Medium-100m-1km) 
        (cloud-cleared yes) (sensitivity-in-low-troposphere-PBL High)  (Temporal-resolution Highest-1h-orless) (taken-by (str-cat ?ins1 "-syn-" ?ins2 "-syn-" ?ins3))
         (Id (str-cat ?id1 "-syn-" ?id2 "-syn" ?id3)) (synergy-level# 2))
    )
...
(defrule SYNERGIES::column-and-profile-ozone-measurements
    ...
    (duplicate ?col (Accuracy# 0.03) (taken-by (str-cat ?ins1 "-syn-" ?ins2))
         (Id (str-cat ?id1 "-syn-" ?id2 )) (synergy-level# 1))
