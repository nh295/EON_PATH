...
for i=1:narch
    arch = archs(i,:);
    tic;
    for j = 1:length(params.nsats)
        fovs = get_fovs(arch,uFOVS);% numeric fov array with -1 if empty orbit
        fprintf('Precomputing arch (%d,%d) from (%d,%d)...\n',i,j,narch,length(params.nsats));
        therevtimes = analyze_mixed_constellation(fovs,params.nsats(j),stk_params,params);
        revtimes = put_revtimes(revtimes,fovs,params.nsats(j),therevtimes);
    end
    t = toc;
    sum = sum + t/60;
    done = i;
...
function key = create_key(fovs,nsat)
    key = [num2str(nsat) 'x' tostringwithcommas(fovs)];
end
function str = tostringwithcommas(vec)
    str = num2str(vec(1));
    for ii = 2:length(vec)
...
function revtimes = analyze_mixed_constellation(fovs,nsat,stk_params,params)
